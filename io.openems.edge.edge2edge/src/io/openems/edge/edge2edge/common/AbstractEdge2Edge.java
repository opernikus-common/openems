...
public abstract class AbstractEdge2Edge extends AbstractOpenemsModbusComponent
	...
	protected boolean activate(ComponentContext context, String id, String alias, boolean enabled, int unitId,
		...
		this.isOpenems().thenAccept(isOpenems -> {
			...
			readFC3HoldingRegisterElementOnce(this.modbusProtocol, ModbusUtils::retryOnNull, new UnsignedWordElement(1))
					.thenAccept(value -> {
						if (value == null) {
							return;
						}
	...
	private CompletableFuture<Boolean> isOpenems() {
		return readFC3HoldingRegisterElementOnce(this.modbusProtocol, ModbusUtils::retryOnNull, new UnsignedWordElement(0)) //
	...
	private void _findComponentBlock(CompletableFuture<Integer> result, String componentId, int startAddress) {
		readFC3HoldingRegisterElementOnce(this.modbusProtocol, ModbusUtils::retryOnNull, new StringWordElement(startAddress, 16)) //
				.thenAccept(remoteComponentId -> {
					...
					readFC3HoldingRegisterElementOnce(this.modbusProtocol, ModbusUtils::retryOnNull,
	...
	private CompletableFuture<Void> readNatureBlocks(int startAddress) {
		return readFC3HoldingRegisterElementOnce(this.modbusProtocol, ModbusUtils::doNotRetry, new UnsignedWordElement(startAddress + 16))
	...
	private static ModbusElement generateModbusElement(ModbusType type, int address) {
		...
		case FLOAT64 -> new FloatQuadruplewordElement(address); // oEMS
	...
	private void _readNatureStartAddresses(CompletableFuture<TreeMap<Integer, Short>> result, int startAddress,
		...
		readFC3HoldingRegisterElementOnce(this.modbusProtocol, ModbusUtils::retryOnNull, new UnsignedWordElement(startAddress))
				.thenAccept(rawHash -> {
					if (rawHash == null) {
					...
					readFC3HoldingRegisterElementOnce(this.modbusProtocol, ModbusUtils::doNotRetry,
