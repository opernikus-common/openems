...
public class AlertingImpl extends AbstractOpenemsBackendComponent implements EventHandler {
	...
	private static final Executor createDefaultExecutorService() {
		final var threadFacotry = new ThreadFactoryBuilder() //
				.setNameFormat(AlertingImpl.class.getSimpleName() + ".EventHandler-%d") //
				.build();
		final var blockingQueue = new LinkedBlockingQueue<Runnable>();
		final var alertingLog = LoggerFactory.getLogger(AlertingImpl.class.getCanonicalName() + "::ThreadPoolExecutor");
	...
	private final Logger log = LoggerFactory.getLogger(AlertingImpl.class);
	...
	protected AlertingImpl(Scheduler scheduler, Executor executor) {
		super("Alerting");
	...
	public AlertingImpl() {
		this(new Scheduler(), AlertingImpl.createDefaultExecutorService());
	}
