...
public class SumStateHandler implements Handler<SumStateMessage> {
	...
	public void send(ZonedDateTime sentAt, List<SumStateMessage> pack) {
		...
		pack.removeIf((msg) -> !this.isEdgeAtLeastAtMessageState(msg));
	...
	private boolean isEdgeAtLeastAtMessageState(SumStateMessage msg) {
		var edgeId = msg.getEdgeId();
		var sumState = this.metadata.getSumState(edgeId);
		return sumState.map(level -> level.isAtLeast(msg.getSumState())).orElse(false);
	}
	...
	private void handleEdgeOnSetSumState(EventReader event) {
		final var edge = (Edge) event.getProperty(Edge.Events.OnSetSumState.EDGE);
